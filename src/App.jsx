...
const SME_DECISION_CATS = [
  ...
  {id:"ops",         label:"Ops. & Alerts",           icon:"⚙️", color:"#f59e0b"},
...
Your role: Give direct, specific, quantified answers to business questions. Always recommend a concrete action with an estimated ${industry.currency} impact. Keep responses under 200 words. Use plain English — no jargon. Format with short paragraphs and bullet points where helpful.`;
    ...
    try{
      ...
      setMessages(p=>[...p,{role:"assistant",content:result.text}]);
      // Auto-surface WhatsApp sender if response is promo-related
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:14,alignItems:"start"}}>
        ...
        <div style={scard()}>
          ...
          {activeCat==="ops"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {items.map((alert,i)=>{
                ...
                const key=`ops-${i}`;
                return(
                  <div key={i} style={{background:SM.bgSec,borderRadius:8,padding:"12px 14px",border:`1px solid ${c}30`,position:"relative"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                      <div style={{flex:1}}>
                        ...
                        <div style={{marginTop:8,background:`${c}10`,borderRadius:5,padding:"7px 10px",border:`1px solid ${c}20`}}>
                          <span style={{fontSize:10,fontWeight:700,color:c}}>⚡ Action: </span>
                          <span style={{fontSize:10,color:SM.muted}}>{alert.action}</span>
                        </div>
                      </div>
                      <button
                        onClick={()=>!executed[key]&&handleExecute(alert,i)}
                        style={{...sbtn(executed[key]?SM.muted:c,{fontSize:10,padding:"5px 10px",flexShrink:0,opacity:executed[key]?0.6:1,background:executed[key]?SM.bgSec:c,border:`1px solid ${c}40`,color:executed[key]?SM.muted:"#fff"})}}>
                        {executed[key]?"✓ In Tracker":"Execute"}
                      </button>
                    </div>
                  </div>
                );
          ...
          {activeCat==="feedback"&&(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {items.map((fb,i)=>{
                const key=`feedback-${i}`;
                return(
                  <div key={i} style={{background:SM.bgSec,borderRadius:8,padding:"12px 14px",border:`1px solid ${SM.border}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                          <span style={{fontSize:12,fontWeight:700,color:SM.text}}>{fb.source}</span>
                          <span style={{fontSize:12,fontWeight:800,color:"#8b5cf6"}}>{fb.rating}</span>
                        ...
                        <div style={{background:"rgba(139,92,246,0.08)",borderRadius:5,padding:"8px 10px",border:"1px solid rgba(139,92,246,0.2)"}}>
                          <span style={{fontSize:10,fontWeight:700,color:"#8b5cf6"}}>💡 Insight: </span>
                          <span style={{fontSize:10,color:SM.muted,lineHeight:1.5}}>{fb.insight}</span>
                        </div>
                      </div>
                      <button
                        onClick={()=>!executed[key]&&handleExecute(fb,i)}
                        style={{...sbtn(executed[key]?SM.muted:"#8b5cf6",{fontSize:10,padding:"5px 10px",flexShrink:0,opacity:executed[key]?0.6:1,background:executed[key]?SM.bgSec:"#8b5cf6",color:executed[key]?SM.muted:"#fff"})}}>
                        {executed[key]?"✓ In Tracker":"Execute"}
                      </button>
                    </div>
                  </div>
                ...
                return(
                  <div key={i} style={{background:SM.bgSec,borderRadius:8,padding:"12px 14px",border:`1px solid ${SM.border}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap"}}>
                          <span style={{fontSize:11,fontWeight:800,color:cat.color,fontFamily:"'JetBrains Mono',monospace",flexShrink:0}}>{String(i+1).padStart(2,"0")}</span>
                          <span style={{fontSize:12,fontWeight:700,color:SM.text}}>{dec.q}</span>
                        </div>
                        <div style={{background:`${cat.color}10`,borderRadius:6,padding:"9px 12px",border:`1px solid ${cat.color}25`,marginBottom:dec.promo?8:0}}>
                          ...
                          {dec.impact&&<div style={{marginTop:5,fontSize:11,fontWeight:800,color:SM.green}}>Impact: {dec.impact}</div>}
                        </div>
                        {dec.promo&&(
                          <WhatsAppSender industry={industry} trigger="dashboard" context={dec.promo} tier="growth"/>
                        )}
                        <button onClick={()=>setPage("copilot")} style={{marginTop:8,fontSize:10,color:cat.color,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit"}}>
                          Ask follow-up →
                        </button>
                      </div>
                      <button
                        onClick={()=>!executed[key]&&handleExecute(dec,i)}
                        style={{...sbtn(executed[key]?SM.muted:cat.color,{fontSize:10,padding:"6px 12px",flexShrink:0,opacity:executed[key]?0.6:1,background:executed[key]?SM.bgSec:cat.color,border:`1px solid ${cat.color}40`,color:executed[key]?SM.muted:"#fff"})}}>
                        {executed[key]?"✓ In Tracker":"Choose to Execute"}
                      </button>
                    </div>
                  </div>
                );
...
function SmeCopilot({industry}){
  ...
  const[extSources,setExtSources]=useState([
    {id:"booking",  label:"Booking System", icon:"📅",active:false,desc:"Live appointment & utilisation data"},
    {id:"reviews",  label:"Google Reviews", icon:"⭐",active:false,desc:"Patient feedback & rating trends"},
    {id:"benchmark",label:"SEA Benchmarks", icon:"📊",active:false,desc:"Dental industry norms SEA"},
    {id:"macro",    label:"Macro (World Bank)",icon:"🌍",active:false,desc:"Live SG/MY economic data"},
  ]);
  ...
  const toggleExt=async(id)=>{
    const src=extSources.find(s=>s.id===id);
    const nowActive=!src.active;
    setExtSources(p=>p.map(s=>s.id===id?{...s,active:nowActive}:s));
    if(nowActive&&id==="macro"){
      setExtLoading(true);
      const gdp=await fetchWorldBankSEA("NY.GDP.MKTP.KD.ZG","GDP Growth %");
      setExtLoading(false);
    }
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:SM.bg}}>
      ...
      <div style={{padding:"10px 20px",borderBottom:`1px solid ${SM.border}`,background:SM.bgCard,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7}}>
          <div style={{fontSize:11,fontWeight:700,color:SM.text}}>+ Add External Data</div>
          {extLoading&&<span style={{...{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:SM.accent}}}>⟳ Fetching...</span>}
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {extSources.map(s=>(
            <button key={s.id} onClick={()=>toggleExt(s.id)} title={s.desc} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 10px",borderRadius:5,fontSize:10,fontWeight:500,cursor:"pointer",border:"none",transition:"all 0.15s",background:s.active?T.primaryDim:SM.bgSec,color:s.active?T.primary:SM.muted,outline:s.active?`1px solid ${T.primary}50`:`1px solid ${SM.border}`}}>
              {s.icon} {s.label} <span style={{fontSize:8,fontFamily:"'JetBrains Mono',monospace",color:s.active?T.primary:SM.muted}}>{s.active?"ON":"OFF"}</span>
            </button>
          ))}
        </div>
      </div>
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:20,background:SM.bg,display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={scard()}>
        ...
        {thisMonth.length===0&&<div style={{fontSize:11,color:SM.muted,textAlign:"center",padding:"20px 0"}}>No initiatives executed yet. Go to Dashboard → Choose to Execute on any decision.</div>}
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {thisMonth.map((r,i)=>{
      ...
      <div style={{...scard(),background:`${industry.color}08`,border:`1px solid ${industry.color}20`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
          {["📤 Upload data","❓ Ask Aether","✅ Choose to Execute","📊 Track MoM outcome"].map((s,i)=>(
