    ...
    <div style={{flex:1,overflowY:"auto",padding:20,background:SM.bg,display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={scard()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          {[
            {label:"This Month",val:thisTotal,c:industry.color,sub:`${allItems.length} decisions tracked`},
              ...
              <div key={i} style={{background:SM.bgSec,borderRadius:8,border:`1px solid ${hasActual?SM.green:SM.border}`,overflow:"hidden",transition:"border-color 0.3s"}}>
                ...
                {isLogging&&(
                  <div style={{padding:"10px 12px",background:`${industry.color}08`,borderTop:`1px solid ${SM.border}`,animation:"fadeIn 0.2s ease"}}>
                    ...
                    <div style={{fontSize:9,color:SM.muted,marginTop:4}}>This updates your ROI total and helps Aether improve future estimates for your business.</div>
                  </div>
                )}
              </div>
            );
      ...
      {learnings.length>0&&(
        <div style={{...scard(),border:`1.5px solid ${SM.purple}30`,background:`${SM.purple}08`}}>
                ...
                <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                      ...
                      <div style={{background:`${SM.purple}15`,borderRadius:4,padding:"3px 8px"}}><span style={{fontSize:9,color:SM.purple}}>Adjusted to: </span><span style={{fontSize:10,fontWeight:800,color:SM.purple}}>{l.adjustedEstimate}</span></div>
          ...
          <div style={{marginTop:10,padding:"8px 12px",background:SM.bgCard,borderRadius:6,border:`1px solid ${SM.border}`}}>
            <div style={{fontSize:10,color:SM.muted,lineHeight:1.55}}><span style={{fontWeight:700,color:SM.purple}}>How this works: </span>Aether compares estimated vs actual outcomes. When actuals differ by more than 15%, it adjusts baseline estimates for similar future initiatives — making recommendations more accurate over time.</div>
          </div>
        </div>
      )}
      ...
      <div style={{...scard(),background:`${industry.color}08`,border:`1px solid ${industry.color}20`}}>
        <div style={{fontSize:11,fontWeight:700,color:industry.color,marginBottom:8}}>How Aether Tracks & Learns</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
          {["📤 Execute decision","📊 Aether estimates impact","✅ Log actual result","🧠 Platform learns","🎯 Better estimates next time"].map((s,i)=>(
            <div key={i} style={{background:SM.bgCard,borderRadius:6,padding:"8px",textAlign:"center",border:`1px solid ${SM.border}`}}>
              <div style={{fontSize:10,color:SM.text,lineHeight:1.4}}>{s}</div>
            </div>
          ))}
        </div>
      </div>
...
function SoloBusinessHealth({industry, bizHealth, setBizHealth}){
  ...
  const addExpense=()=>{
    ...
    const newExp={id:`e-${Date.now()}`,label:expInput.trim(),amount:parseFloat(expAmount)||0,cat:expCat,date:new Date().toISOString().slice(0,10)};
    setBizHealth(p=>({...p,expenses:[...(p.expenses||[]),newExp]}));
    setExpInput(""); setExpAmount(""); setExpCat("marketing");
  };
  ...
  const channelSpend={};
  expenses.filter(e=>e.cat==="marketing").forEach(e=>{
  ...
  const generateInsight=async()=>{
    ...
    const system=`You are Aether Solo — a business intelligence advisor for ${industry.bizName}, a ${industry.role}.

Analyse their marketing and expense data. Give 3 specific, actionable insights:
1. Which channel is delivering the best ROI and should receive more investment
2. Which spend is delivering zero return and should be cut or reduced
3. One specific action they should take THIS WEEK based on the data

Be direct and commercial. Reference actual numbers. Max 200 words total.`;
    const ctx=`Revenue: ${cur}${revenue} | Total expenses: ${cur}${totalSpend} | Marketing spend: ${cur}${mktSpend} (${mktPct}% of revenue) | Net profit: ${cur}${netProfit}
Channel performance: ${channelData}
Expenses breakdown: ${expenses.map(e=>`${e.label} ${cur}${e.amount} (${e.cat})`).join(", ")||"None logged yet"}`;
    try{
      const result=await callClaude([{role:"user",content:ctx}],system,false,"insight");
      setInsight(result.text);
    }catch(e){
      setInsight("Add VITE_ANTHROPIC_KEY in Vercel to generate marketing insights.");
    }
    setLoadingInsight(false);
  };

  const catColors={marketing:SL.tealL,tools:SL.purple,operations:SL.amber,professional:SL.blue};
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:20,background:SL.bg,display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* P&L summary header */}
      <div style={{...slcard(),background:"#080c18",border:`1px solid ${SL.border}`}}>
        <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:2,textTransform:"uppercase",fontFamily:"'JetBrains Mono',monospace",marginBottom:10}}>Business Health — This Month</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12}}>
          {[
            {l:"Revenue",     v:`${cur}${revenue.toLocaleString()}`,       c:"34d399"},
            {l:"Total Spend", v:`${cur}${totalSpend.toLocaleString()}`,     c:"f87171"},
            ...
            <div key={s.l}>
              <div style={{fontSize:18,fontWeight:900,color:`#${s.c}`,fontFamily:"'JetBrains Mono',monospace",letterSpacing:-0.5}}>{s.v}</div>
        ...
        {parseFloat(mktPct)>0&&(
          <div style={{marginTop:10,padding:"6px 10px",borderRadius:5,background:parseFloat(mktPct)>15?"rgba(239,68,68,0.1)":parseFloat(mktPct)>8?"rgba(245,158,11,0.1)":"rgba(16,185,129,0.1)",border:`1px solid ${parseFloat(mktPct)>15?"rgba(239,68,68,0.3)":parseFloat(mktPct)>8?"rgba(245,158,11,0.3)":"rgba(16,185,129,0.3)"}`}}>
            <span style={{fontSize:10,color:parseFloat(mktPct)>15?SL.red:parseFloat(mktPct)>8?SL.amber:SL.green,fontWeight:600}}>
              {parseFloat(mktPct)>15?"⚠️ Marketing spend above 15% of revenue — review channel ROI":parseFloat(mktPct)>8?"📊 Marketing at "+mktPct+"% of revenue — healthy range (5–15%)":"✓ Marketing spend efficient at "+mktPct+"% of revenue"}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,alignItems:"start"}}>
        ...
        {/* Expense logger */}
        <div style={slcard()}>
          <div style={{fontSize:12,fontWeight:800,color:SL.text,marginBottom:4}}>Log Expenses</div>
          <div style={{fontSize:11,color:SL.muted,marginBottom:12}}>Track spend by category — Aether learns what generates return</div>
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
            <input value={expInput} onChange={e=>setExpInput(e.target.value)} placeholder="e.g. Facebook ads April, Adobe CC, Canva Pro..."
              style={{background:SL.bgSec,border:`1px solid ${SL.border}`,borderRadius:6,padding:"7px 10px",fontSize:11,color:SL.text,outline:"none",fontFamily:"inherit"}}/>
            <div style={{display:"flex",gap:6}}>
              <input value={expAmount} onChange={e=>setExpAmount(e.target.value)} placeholder={`Amount (${cur})`} type="number"
                style={{flex:1,background:SL.bgSec,border:`1px solid ${SL.border}`,borderRadius:6,padding:"7px 10px",fontSize:11,color:SL.text,outline:"none",fontFamily:"inherit"}}/>
              <select value={expCat} onChange={e=>setExpCat(e.target.value)}
                style={{flex:1,background:SL.bgSec,border:`1px solid ${SL.border}`,borderRadius:6,padding:"7px 8px",fontSize:11,color:SL.text,fontFamily:"inherit",cursor:"pointer"}}>
                <option value="marketing">Marketing</option>
                <option value="tools">Tools & Subs</option>
                <option value="operations">Operations</option>
                <option value="professional">Professional</option>
              </select>
            </div>
            <button onClick={addExpense} disabled={!expInput.trim()||!expAmount} style={{...slbtn(industry.color,{fontSize:11,justifyContent:"center",opacity:!expInput.trim()||!expAmount?0.4:1})}}>+ Add Expense</button>
          </div>
          {expenses.length===0&&<div style={{fontSize:10,color:SL.dim,textAlign:"center",padding:"8px 0",fontStyle:"italic"}}>No expenses logged yet — add your first one above</div>}
          <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:220,overflowY:"auto"}}>
            {expenses.map(e=>(
              <div key={e.id} style={{display:"flex",gap:8,alignItems:"center",background:SL.bgSec,borderRadius:5,padding:"6px 9px"}}>
                <span style={{...sltag(catColors[e.cat]||SL.dim),fontSize:8}}>{e.cat.toUpperCase()}</span>
                <span style={{flex:1,fontSize:10,color:SL.text}}>{e.label}</span>
                <span style={{fontSize:11,fontWeight:700,color:SL.red,fontFamily:"'JetBrains Mono',monospace"}}>{cur}{e.amount}</span>
                <button onClick={()=>setBizHealth(p=>({...p,expenses:p.expenses.filter(x=>x.id!==e.id)}))} style={{fontSize:11,color:SL.dim,background:"none",border:"none",cursor:"pointer"}}>×</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={slcard()}>
            ...
            <div style={{fontSize:11,color:SL.muted,marginBottom:12}}>Revenue per channel vs what you spent — based on deal source tracking in Pipeline</div>
            {Object.keys(channels).length===0?(
              <div style={{fontSize:10,color:SL.dim,textAlign:"center",padding:"16px 0",fontStyle:"italic"}}>No channel data yet — assign deal sources in the Pipeline tab to see ROI per channel</div>
            ):(
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {Object.entries(channels).sort((a,b)=>b[1].revenue-a[1].revenue).map(([ch,d])=>{
                  ...
                  return(
                    <div key={ch} style={{background:SL.bgSec,borderRadius:6,padding:"8px 10px"}}>
                      <div style={{display:"grid",gridTemplateColumns:"1fr auto auto auto auto",gap:8,alignItems:"center"}}>
                        <span style={{fontSize:11,fontWeight:600,color:SL.text}}>{ch}</span>
                        <span style={{fontSize:10,color:SL.muted,fontFamily:"'JetBrains Mono',monospace"}}>{d.clients} client{d.clients!==1?"s":""}</span>
                        <span style={{fontSize:11,fontWeight:700,color:SL.green,fontFamily:"'JetBrains Mono',monospace"}}>{cur}{d.revenue.toLocaleString()}</span>
                        <span style={{fontSize:10,color:SL.dim,fontFamily:"'JetBrains Mono',monospace"}}>spend: {spend?`${cur}${spend}`:"S$0"}</span>
                        <span style={{...sltag(roiC),fontWeight:800}}>{roi}x ROI</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{...slcard(),marginTop:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div>
                <div style={{fontSize:12,fontWeight:800,color:SL.text}}>🧠 Marketing Intelligence</div>
                <div style={{fontSize:10,color:SL.muted}}>Aether analyses your spend + attribution to suggest strategy</div>
              </div>
              <button onClick={generateInsight} disabled={loadingInsight} style={{...slbtn(industry.color,{fontSize:10,padding:"5px 12px",opacity:loadingInsight?0.6:1})}}>
            ...
            {insight?(
              ...
              <div style={{background:SL.bgSec,borderRadius:7,padding:"14px 12px",textAlign:"center"}}>
                <div style={{fontSize:10,color:SL.dim,fontStyle:"italic"}}>Log expenses + assign deal sources in Pipeline, then click Get Insights for Aether to tell you which channels to invest in and which to cut</div>
              </div>
            )}
          </div>
        </div>
