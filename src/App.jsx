...
// ── SOLO PIPELINE TAB ─────────────────────────────────────────────────────────
function SoloPipeline({industry, contacts, setContacts}){
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {["all",...industry.pipelineStages].map(stage=>(
          <button key={stage} onClick={()=>setActiveStage(stage)} style={{fontSize:11,padding:"5px 14px",borderRadius:20,border:`1.5px solid ${activeStage===stage?industry.color:SL.border}`,background:activeStage===stage?`${industry.color}15`:"transparent",color:activeStage===stage?industry.color:SL.muted,cursor:"pointer",fontFamily:"inherit",fontWeight:activeStage===stage?700:400}}>
            {stage==="all"?"All Deals":stage} {stage!=="all"&&<span style={{fontSize:9,fontFamily:"'JetBrains Mono',monospace"}}>({deals.filter(d=>d.stage===stage).length})</span>}
          </button>
        ))}
      </div>

      {/* Two column layout: pipeline + contacts */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:14,alignItems:"start"}}>

        {/* Deal cards */}
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {sorted.map(deal=>(
            <div key={deal.id} style={{...slcard(),border:`1px solid ${deal.heat==="high"?SL.red+"40":deal.heat==="medium"?SL.amber+"30":SL.border}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                    ...
                    <SLTag c={SL.tealL}>{deal.type}</SLTag>
                  </div>
                  <div style={{fontSize:11,color:SL.muted,lineHeight:1.5,marginBottom:6}}>{deal.note}</div>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                    <span style={{fontSize:10,fontFamily:"'JetBrains Mono',monospace",color:SL.dim}}>Last contact: {deal.lastContact}d ago</span>
                    <span style={{fontSize:10,color:SL.amber,fontFamily:"'JetBrains Mono',monospace"}}>Due: {deal.due}</span>
                    <span style={{fontSize:12,fontWeight:800,color:industry.color,fontFamily:"'JetBrains Mono',monospace"}}>{deal.value}</span>
                  </div>
                  <SoloWhatsAppSend
                    industry={industry}
                    contacts={[...contacts,...industry.deals.map(d=>({id:d.id,name:d.name,phone:d.phone,type:d.type}))]}
                    context={`Follow up with ${deal.name} regarding ${deal.note}. Last contact was ${deal.lastContact} days ago. The deal is ${deal.heat} heat. Stage: ${deal.stage}.`}
        ...
        <div>
          ...
          {/* Pipeline summary */}
          <div style={slcard()}>
            <div style={{fontSize:11,fontWeight:700,color:SL.text,marginBottom:8}}>Pipeline Summary</div>
            {["high","medium","low"].map(h=>{
              const c=h==="high"?SL.red:h==="medium"?SL.amber:SL.dim;
              const label=h==="high"?"HOT":h==="medium"?"WARM":"COOL";
              const count=deals.filter(d=>d.heat===h).length;
              return(
                <div key={h} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 0",borderBottom:`1px solid ${SL.border}`}}>
                  <span style={{...sltag(c)}}>{label}</span>
                  <span style={{fontSize:12,fontWeight:700,color,fontFamily:"'JetBrains Mono',monospace"}}>{count} deal{count!==1?"s":""}</span>
                </div>
              );
            })}
          </div>
      ...
      <div style={{height:"100vh",background:SL.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,fontFamily:"'Inter',system-ui,sans-serif",color:SL.text}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,width:"100%",maxWidth:780}}>
          {SOLO_INDUSTRIES.map(ind=>(
            <button key={ind.id} onClick={()=>{setIndustry(ind);setPage("dashboard");setContacts(ind.deals.map(d=>({id:d.id,name:d.name,phone:d.phone,type:d.type,note:d.note})));}}
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:SL.bg,fontFamily:"'Inter',system-ui,sans-serif",color:SL.text,overflow:"hidden"}}>
      ...
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex"}}>
          {page==="dashboard"&&<SoloDashboard industry={industry}/>}
          {page==="pipeline" &&<SoloPipeline  industry={industry} contacts={contacts} setContacts={setContacts}/>}
          {page==="engage"   &&<SoloEngagement industry={industry} contacts={contacts}/>}
          {page==="calendar" &&<SoloCalendar  industry={industry}/>}
          {page==="copilot"  &&<SoloCopilot   industry={industry}/>}
        </div>
      </main>
    </div>
  );
