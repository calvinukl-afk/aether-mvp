...
function SmeROITracker({industry, executedItems=[], onActualLogged, learnings=[], campaigns=[]}){
  ...
  const SNAPSHOTS_KEY=`aether_sme_snapshots_${industry.id}`;
  ...
  const[expCat,setExpCat]=useState("staffcost");
  const[expDate,setExpDate]=useState(new Date().toISOString().slice(0,10));
  const[expNote,setExpNote]=useState("");
  const[closingMonth,setClosingMonth]=useState(false);

  const CATS=[
    {id:"staffcost",   label:"Staff Costs (Salaries, CPF)",ded:100,color:SM.accent},
    ...
    {id:"professional",label:"Professional Services",      ded:100,color:SM.blue},
    {id:"equipment",   label:"Equipment & Maintenance",    ded:100,color:SM.muted},
    {id:"entertainment",label:"Client Entertainment",      ded:50, color:SM.red, flag:"50% deductible"},
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        const r2=await window.storage.get(SNAPSHOTS_KEY);
        if(r1) setAllExpenses(JSON.parse(r1.value));
        if(r2) setSnapshots(JSON.parse(r2.value));
      }catch(e){}
      setLoaded(true);
    })();
  },[industry.id]);
  ...
  const saveExpenses=(exps)=>{
    setAllExpenses(exps);
    window.storage.set(STORAGE_KEY,JSON.stringify(exps)).catch(()=>{});
  };
  const saveSnapshots=(snaps)=>{
    setSnapshots(snaps);
    window.storage.set(SNAPSHOTS_KEY,JSON.stringify(snaps)).catch(()=>{});
  };

  // ── Computed ─────────────────────────────────────────────────────────────
  const prevMonthTotal=industry.roiItems.reduce((s,r)=>s+parseFloat((r.prevMonth||"0").replace(/[^0-9.]/g,"")),0);
  ...
  const fmtMonth=(ym)=>{const[y,m]=ym.split("-");return new Date(+y,+m-1).toLocaleDateString("en-SG",{month:"short",year:"numeric"});};

  const submitActual=(item)=>{
    const val=(actualInputs[item.id]||"").trim();
    if(!val)return;
    const formatted=val.startsWith(industry.currency)?val:`${industry.currency}${val}`;
    if(onActualLogged) onActualLogged(item.id,formatted,"");
    setLoggingId(null);
    setActualInputs(p=>({...p,[item.id]:""}));
  };
  ...
  const exportCSV=()=>{
    ...
    rows.push(`"","YTD Total",,,${totalYTD.toFixed(2)},`);
    rows.push(`"","Estimated Deductible YTD",,,${deductYTD.toFixed(2)},`);
    rows.push(`"","For reference only — consult a tax professional for your actual filing",,,,`);
    const blob=new Blob([rows.join("\n")],{type:"text/csv"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url; a.download=`${year}_Business_Expenses_${industry.bizName.replace(/\s+/g,"_")}.csv`;
  ...
  const closeMonth=async()=>{
    ...
    const system=`You are Aether SME — a business analyst for ${industry.bizName} (${industry.label}, ${industry.location}).
Write a concise monthly business review. Format as 3 sections with bold headers:
**Revenue & Decisions** — what was executed, estimated vs actual impact
**Expenses & Profitability** — spend breakdown, key cost observations  
**Priority for Next Month** — one specific lever to pull, with expected impact
Max 200 words. Be specific with numbers. Tone: like a CFO presenting a board update.`;
    const ctx=`Month: ${fmtMonth(activeMonth)}
Decisions executed: ${executedItems.length} | Value tracked: ${industry.currency}${thisTotal.toLocaleString()} | vs last month: ${momDelta>=0?"+":`-`}${industry.currency}${Math.abs(momDelta).toLocaleString()}
Actual results logged: ${executedItems.filter(i=>i.actualValue).length} of ${executedItems.length}
Expenses this month: ${industry.currency}${totalMonth.toLocaleString()}
Expense breakdown: ${CATS.map(c=>{const t=monthExps.filter(e=>e.cat===c.id).reduce((s,e)=>s+e.amount,0);return t>0?`${c.label}: ${industry.currency}${t}`:null;}).filter(Boolean).join(", ")||"No expenses logged"}
Platform learnings: ${learnings.length} adjustments made`;
    let summary="";
    try{const r=await callClaude([{role:"user",content:ctx}],system,false,"snapshot");summary=r.text;}
    catch(e){summary=`**Revenue & Decisions**\n${executedItems.length} decisions executed · Value tracked: ${industry.currency}${thisTotal.toLocaleString()}\n\n**Expenses**\nTotal expenses: ${industry.currency}${totalMonth.toLocaleString()}\n\n**Next Month**\nAdd VITE_ANTHROPIC_KEY to generate AI-written monthly review.`;}
    const snap={id:activeMonth,month:activeMonth,label:fmtMonth(activeMonth),summary,
                decisionsExecuted:executedItems.length,valueTracked:thisTotal,
                expenses:totalMonth,momDelta,closedAt:new Date().toISOString()};
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:20,background:SM.bg,display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* Tab switcher */}
      <div style={{display:"flex",gap:6,background:SM.bgCard,borderRadius:8,padding:4,width:"fit-content",border:`1px solid ${SM.border}`}}>
      ...
      {activeTab==="roi"&&(<>
        ...
        {pendingCheckin.length>0&&(
          <div style={{background:`${SM.amber}15`,border:`1.5px solid ${SM.amber}40`,borderRadius:10,padding:"12px 16px",display:"flex",gap:12,alignItems:"flex-start",animation:"fadeIn 0.3s ease"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:SM.muted,lineHeight:1.5,marginBottom:8}}>Log the actual impact below. This helps Aether learn and improve future estimates for your business.</div>
        ...
        <div style={{background:"#080c18",border:`1px solid ${SM.border}`,borderRadius:10,padding:20,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
            <div>
              ...
              <div style={{fontSize:9,color:momDelta>=0?"#34d399":"#f87171",marginTop:3,fontFamily:"'JetBrains Mono',monospace",fontWeight:700}}>{momDelta>=0?"▲":"▼"} {industry.currency}{Math.abs(momDelta).toLocaleString()} vs last month</div>
            </div>
            <div>
              <div style={{fontSize:26,fontWeight:900,color:"#60a5fa",letterSpacing:-1,fontFamily:"'JetBrains Mono',monospace"}}>{Math.round(thisTotal/fee)}x</div>
            ...
            <div>
              <div style={{fontSize:26,fontWeight:900,color:SM.amber,letterSpacing:-1,fontFamily:"'JetBrains Mono',monospace"}}>{industry.currency}{prevMonthTotal.toLocaleString()}</div>
        ...
        <div style={scard()}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            {[{label:"This Month",val:thisTotal,c:industry.color,sub:`${allItems.length} decisions tracked`},{label:"Last Month",val:prevMonthTotal,c:SM.muted,sub:`${industry.roiItems.length} decisions tracked`}].map(col=>(
        ...
        <div style={scard()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {allItems.map((r,i)=>{
              ...
              return(
                <div key={i} style={{background:SM.bgSec,borderRadius:8,border:`1px solid ${hasActual?SM.green:SM.border}`,overflow:"hidden",transition:"border-color 0.3s"}}>
                  <div style={{padding:"10px 12px",display:"flex",alignItems:"flex-start",gap:10}}>
                    <div style={{width:34,height:34,borderRadius:7,background:`${typeColor}15`,border:`1px solid ${typeColor}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      {(hasActual||r.estimatedValue||r.value)&&(
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:4}}>
                          ...
                          <div style={{background:hasActual?`${SM.green}15`:SM.bgCard,borderRadius:5,padding:"5px 8px",border:hasActual?`1px solid ${SM.green}30`:"none"}}>
                  ...
                  {isLogging&&(
                    <div style={{padding:"10px 12px",background:`${industry.color}08`,borderTop:`1px solid ${SM.border}`,animation:"fadeIn 0.2s ease"}}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
                        <div style={{flex:1}}>
                          <div style={{fontSize:9,color:SM.muted,marginBottom:3}}>Actual value delivered ({industry.currency})</div>
        ...
        {/* Platform Learning */}
        {learnings.length>0&&(
          <div style={{...scard(),border:`1.5px solid ${SM.purple}30`,background:`${SM.purple}08`}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              <span style={{fontSize:18}}>🧠</span>
              <div>
                <div style={{fontSize:12,fontWeight:800,color:SM.purple}}>Aether Platform Learning</div>
                <div style={{fontSize:10,color:SM.muted}}>Based on your actual results, Aether has updated its estimates for your business</div>
              </div>
              <STag c={SM.purple}>{learnings.length} INSIGHT{learnings.length>1?"S":""}</STag>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {learnings.map((l,i)=>(
                <div key={i} style={{background:SM.bgCard,borderRadius:8,padding:"10px 14px",border:`1px solid ${SM.purple}20`}}>
                  <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:10,color:SM.muted,lineHeight:1.6,marginBottom:8}}>{l.learning}</div>
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        <div style={{background:`${SM.purple}15`,borderRadius:4,padding:"3px 8px"}}><span style={{fontSize:9,color:SM.purple}}>Adjusted: </span><span style={{fontSize:10,fontWeight:800,color:SM.purple}}>{l.adjustedEstimate}</span></div>
        ...
        {campaigns.length>0&&(
          <div style={scard()}>
            <div style={{fontSize:12,fontWeight:800,color:SM.text,marginBottom:10}}>Customer Sales Engagement — Campaign Summary</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:10}}>
              {[{l:"Campaigns sent",v:campaigns.length,c:industry.color},{l:"Total link clicks",v:campaigns.reduce((s,c)=>s+(c.clicks||0),0),c:SM.accent},{l:"Revenue tracked",v:`${industry.currency}${campaigns.filter(c=>c.revenue).reduce((s,c)=>s+parseFloat(c.revenue||"0"),0).toLocaleString()}`,c:SM.green}].map(m=>(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div style={{background:"#080c18",border:`1px solid ${SM.border}`,borderRadius:10,padding:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              ...
              <div><div style={{fontSize:20,fontWeight:900,color:"#34d399",fontFamily:"'JetBrains Mono',monospace"}}>{industry.currency}{deductYTD.toLocaleString()}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.4)",marginTop:2}}>Est. Deductible YTD</div></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,alignItems:"start"}}>
            ...
            <div style={scard()}>
              ...
              <div style={{fontSize:10,color:SM.muted,marginBottom:10}}>Saved permanently · Full year CSV export for tax filing</div>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                <input value={expInput} onChange={e=>setExpInput(e.target.value)} placeholder="Description"
                  style={{background:SM.bgSec,border:`1px solid ${SM.border}`,borderRadius:5,padding:"6px 9px",fontSize:11,color:SM.text,outline:"none",fontFamily:"inherit"}}/>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  <input value={expAmount} onChange={e=>setExpAmount(e.target.value)} placeholder={`Amount (${industry.currency})`} type="number"
                    style={{background:SM.bgSec,border:`1px solid ${SM.border}`,borderRadius:5,padding:"6px 9px",fontSize:11,color:SM.text,outline:"none",fontFamily:"inherit"}}/>
                ...
                <select value={expCat} onChange={e=>setExpCat(e.target.value)}
                  style={{background:SM.bgSec,border:`1px solid ${SM.border}`,borderRadius:5,padding:"6px 9px",fontSize:11,color:SM.text,fontFamily:"inherit",cursor:"pointer"}}>
            ...
            <div style={scard()}>
              ...
              {monthExps.length===0&&<div style={{fontSize:10,color:SM.dim,textAlign:"center",padding:"16px 0",fontStyle:"italic"}}>No expenses logged for this month</div>}
                ...
                {monthExps.sort((a,b)=>b.date.localeCompare(a.date)).map(e=>{
                  ...
                  return(
                    <div key={e.id} style={{display:"flex",gap:7,alignItems:"center",background:SM.bgSec,borderRadius:5,padding:"5px 9px"}}>
                      <span style={{fontSize:7.5,padding:"2px 6px",borderRadius:3,background:`${cat?.color||SM.dim}15`,color:cat?.color||SM.dim,fontFamily:"'JetBrains Mono',monospace",fontWeight:700,flexShrink:0}}>{cat?.label?.split(" ")[0]?.toUpperCase()||e.cat.toUpperCase()}</span>
                      <div style={{flex:1,minWidth:0}}>
                      ...
                      <button onClick={()=>saveExpenses(allExpenses.filter(x=>x.id!==e.id))} style={{fontSize:11,color:SM.dim,background:"none",border:"none",cursor:"pointer"}}>×</button>
                    </div>
                  );
                })}
              </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={closeMonth} disabled={closingMonth||!!thisSnap} style={{...sbtn(industry.color,{fontSize:10,padding:"7px 14px",opacity:closingMonth||thisSnap?0.5:1})}}>
              {closingMonth?"Generating...":`${thisSnap?"✓ Month Closed":"Close "+fmtMonth(activeMonth)}`}
            </button>
          </div>
          {snapshots.length===0&&(
            <div style={{...scard(),textAlign:"center",padding:"32px 20px"}}>
              ...
              <div style={{fontSize:11,color:SM.muted}}>Click "Close {fmtMonth(activeMonth)}" to generate your first monthly business review. Aether will analyse your decisions, ROI, and expenses to produce a permanent record.</div>
            </div>
          )}
          {snapshots.map(snap=>(
            <div key={snap.id} style={{...scard(),border:`1px solid ${industry.color}30`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,paddingBottom:10,borderBottom:`1px solid ${SM.border}`}}>
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontSize:10,color:SM.muted}}>{snap.decisionsExecuted||0} decisions</span>
                  <STag c={SM.green}>{industry.currency}{snap.valueTracked?.toLocaleString()||"—"}</STag>
              ...
              <div style={{fontSize:8.5,color:SM.dim,marginTop:8}}>Closed {new Date(snap.closedAt).toLocaleDateString("en-SG",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}</div>
            </div>
          ))}
        </div>
      )}
