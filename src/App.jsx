...
// ── SOLO CO-PILOT ─────────────────────────────────────────────────────────────
function SoloCopilot({industry, bizHealth={}}){
  const[allFiles,setAllFiles]=useState([]);
  const[pending,setPending]=useState([]);
  const[dragOver,setDragOver]=useState(false);
  const[activeQ,setActiveQ]=useState(null);
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:SL.bg}}>
      ...
      <div style={{flex:1,padding:20,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        <SoloChatBox industry={industry} initialMsg={activeQ?`## ${activeQ}\n\nLet me look at your pipeline data...`:welcomeMsg} pendingFiles={pending} setPending={setPending} bizHealth={bizHealth}/>
      </div>
    </div>
  );
}

function SoloChatBox({industry, initialMsg, pendingFiles=[], setPending, bizHealth={}}){
  const[messages,setMessages]=useState([{role:"assistant",content:initialMsg}]);
  const[input,setInput]=useState("");
  const[loading,setLoading]=useState(false);
  const endRef=useRef(null);
...
Pipeline: ${industry.deals.map(d=>`${d.name} (${d.stage}, ${d.heat} heat, last contact ${d.lastContact}d ago): ${d.note}`).join(" | ")}
KPIs: ${industry.kpis.map(k=>`${k.label}: ${k.value}`).join(" · ")}

Give direct, specific, quantified answers. Reference actual deal names and numbers from their pipeline. Recommend the single best action they can take today. Keep under 200 words. Plain English, no jargon.`;
...
// ── SOLO KPI DASHBOARD ────────────────────────────────────────────────────────
function SoloDashboard({industry}){
  return(
    <div style={{flex:1,overflowY:"auto",padding:20,background:SL.bg,display:"flex",flexDirection:"column",gap:14}}>
      <div style={{fontSize:11,fontWeight:700,color:SL.muted,letterSpacing:2,textTransform:"uppercase",fontFamily:"'JetBrains Mono',monospace"}}>My Numbers — This Month</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
        {industry.kpis.map(k=>(
          <div key={k.label} style={{...slcard(),position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg,${industry.color},transparent)`}}/>
            <div style={{fontSize:9,fontWeight:700,color:SL.muted,letterSpacing:1.5,textTransform:"uppercase",fontFamily:"'JetBrains Mono',monospace",marginBottom:6}}>{k.label}</div>
            <div style={{fontSize:22,fontWeight:800,color:SL.text,letterSpacing:-0.5,marginBottom:2}}>{k.value}</div>
            <span style={{fontSize:11,fontWeight:700,color:k.pos?SL.green:SL.red,fontFamily:"'JetBrains Mono',monospace"}}>{k.change}</span>
            <div style={{fontSize:10,color:SL.dim,marginTop:3}}>{k.sub}</div>
          </div>
      ...
      {/* Hot leads today */}
      <div style={slcard()}>
        <div style={{fontSize:12,fontWeight:800,color:SL.text,marginBottom:10}}>⚡ Action Required Today</div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {industry.deals.filter(d=>d.heat==="high").map(d=>(
            <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:SL.bgSec,borderRadius:6,padding:"8px 12px",border:`1px solid ${SL.red}30`}}>
              <div>
                <div style={{fontSize:11,fontWeight:700,color:SL.text}}>{d.name} <span style={{...sltag(SL.red),marginLeft:4}}>HOT</span></div>
                <div style={{fontSize:10,color:SL.muted,marginTop:2}}>{d.note.slice(0,70)}...</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0,marginLeft:10}}>
                <div style={{fontSize:12,fontWeight:800,color:industry.color,fontFamily:"'JetBrains Mono',monospace"}}>{d.value}</div>
                <div style={{fontSize:9,color:SL.amber,marginTop:1}}>{d.due}</div>
              </div>
            </div>
          ))}
        </div>
