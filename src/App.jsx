...
function SmeROITracker({industry, executedItems=[], onActualLogged, learnings=[], campaigns=[]}){
  ...
  // ── Derived values ────────────────────────────────────────────────────────
  const prevTotal=industry.roiItems.reduce((s,r)=>s+parseFloat((r.prevMonth||"0").replace(/[^0-9.]/g,"")),0);
  const thisTotal=allItems.reduce((s,r)=>s+parseFloat((r.actualValue||r.value||"0").replace(/[^0-9.]/g,"")),0);
  ...
  const exportCSV=()=>{
    ...
    a.href=url; a.download=year+"_Expenses_"+industry.bizName.replace(/\\s+/g,"_")+".csv";
          ...
          <div style={scard()}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {allItems.map((r,i)=>{
                ...
                const estNum=parseFloat((r.estimatedValue||r.value||"0").replace(/[^0-9.]/g,""));
                const actNum=hasActual?parseFloat((r.actualValue||"0").replace(/[^0-9.]/g,"")):null;
                      ...
                      <div style={{padding:"10px 12px",background:industry.color+"10",borderTop:"1px solid "+SM.border}}>
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
                          <div style={{flex:1}}>
                            ...
                            <input value={actualInputs[r.id]||""} onChange={e=>setActualInputs(p=>({...p,[r.id]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&submitActual(r)} placeholder={"e.g. "+(r.estimatedValue||r.value||"").replace(/[^0-9.,]/g,"")} style={{width:"100%",background:SM.bgCard,border:"1px solid "+SM.border,borderRadius:5,padding:"6px 8px",fontSize:11,color:SM.text,outline:"none",fontFamily:"inherit"}}/>
                          </div>
      ...
      <div style={{...slcard(),background:"#080c18",border:`1px solid ${SL.border}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
          {[
            ...
            {label:"Target",       val:parseFloat(industry.kpis[0].value.replace(/[^0-9.]/g,"")), c:SL.gold, sub:"Monthly income goal"},
          ].map(s=>(
            <div key={s.label}>
...
function SoloChatBox({industry, initialMsg, pendingFiles=[], setPending}){
  ...
  const send=async(text)=>{
    ...
    const revenue=parseFloat(industry.kpis[0].value.replace(/[^0-9.]/g,""))||0;
    const channels={};
    dealSources.forEach(ds=>{if(ds.source){if(!channels[ds.source])channels[ds.source]={clients:0,revenue:0};channels[ds.source].clients++;channels[ds.source].revenue+=parseFloat((ds.value||"0").replace(/[^0-9.]/g,""));}});
...
function SoloBusinessHealth({industry, bizHealth, setBizHealth}){
  ...
  dealSources.forEach(ds=>{
    ...
    channels[ds.source].revenue+=parseFloat((ds.value||"0").replace(/[^0-9.]/g,""));
  });
  const channelSpend={};
  ...
  const exportCSV=()=>{
    ...
    const a=document.createElement("a");
    a.href=url;
    a.download=`${year}_Business_Expenses_${industry.bizName.replace(/\s+/g,"_")}.csv`;
    a.click(); URL.revokeObjectURL(url);
  };
