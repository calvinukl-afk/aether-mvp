      ...
      {activeTab==="roi"&&(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{background:SM.amber+"25",border:"1.5px solid "+SM.amber+"50",borderRadius:10,padding:"12px 16px",display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {pendingCheckin.slice(0,3).map(i=>(
                    <button key={i.id} onClick={()=>setLoggingId(loggingId===i.id?null:i.id)} style={{...sbtn(SM.amber,{fontSize:10,padding:"4px 10px",background:SM.amber+"20",color:SM.amber,border:"1px solid "+SM.amber+"40"})}}>
                      {(i.action||"Initiative").slice(0,28)}... →
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div style={scard()}>
            ...
            {allItems.length===0&&<div style={{fontSize:11,color:SM.muted,textAlign:"center",padding:"20px 0"}}>No initiatives executed yet. Go to Dashboard and click Execute on any decision.</div>}
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {allItems.map((r,i)=>{
                  ...
                  <div key={i} style={{background:SM.bgSec,borderRadius:8,border:"1px solid "+(hasActual?SM.green:SM.border),overflow:"hidden"}}>
                    <div style={{padding:"10px 12px",display:"flex",alignItems:"flex-start",gap:10}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:hasActual||r.estimatedValue?6:0}}>
                          <STag c={tc}>{tl}</STag>
                          {!hasActual&&r.executedAt&&<STag c={SM.amber}>AWAITING RESULTS</STag>}
                          {hasActual&&<STag c={SM.green}>✓ LOGGED</STag>}
                        </div>
            ...
            <div style={scard()}>
              ...
              {campaigns.filter(c=>c.status==="tracked").map((c,i)=>(
                <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"7px 10px",background:SM.bgSec,borderRadius:6,marginBottom:4}}>
                  ...
                  <div style={{flex:1}}><div style={{fontSize:10,fontWeight:600,color:SM.text}}>{c.label}</div><div style={{fontSize:9,color:SM.muted}}>{c.sentAt}</div></div>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
                    {c.clicks>0&&<STag c={SM.accent}>{c.clicks} clicks</STag>}
                    {c.replies!=null&&<STag c="#25D166">{c.replies} replies</STag>}
                    {c.bookings!=null&&<STag c={SM.green}>{c.bookings} bookings</STag>}
                    {c.revenue&&<STag c={SM.green}>{industry.currency}{c.revenue}</STag>}
                  </div>
                </div>
              ))}
            </div>
...
function SmeApp({onBack}){
  ...
  const handleActualLogged=(itemId, actualVal, note)=>{
    setExecutedItems(p=>p.map(i=>{
      ...
      const est=parseFloat((i.estimatedValue||"0").replace(/[^0-9.]/g,""));
      const act=parseFloat((actualVal||"0").replace(/[^0-9.]/g,""));
      if(est>0 && act>0){
        const ratio=act/est;
...
function SoloPipeline({industry, contacts, setContacts, bizHealth, setBizHealth}){
  ...
  const confirmed=deals.filter(d=>["Confirmed","In Progress","Delivered","Closed Won"].includes(d.stage)).reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
  const atRisk=deals.filter(d=>d.heat==="medium"&&["Quote Sent","Proposal Sent"].includes(d.stage)).reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
  const fullPipe=deals.reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
...
function SoloDashboard({industry, bizHealth, weeklyDigest, setWeeklyDigest}){
  ...
  const confirmed = industry.deals.filter(d=>d.stage==="Confirmed"||d.stage==="In Progress"||d.stage==="Delivered"||d.stage==="Closed Won").reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
  const atRisk    = industry.deals.filter(d=>d.heat==="medium"&&(d.stage==="Quote Sent"||d.stage==="Proposal Sent")).reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
  const fullPipe  = industry.deals.reduce((s,d)=>s+parseFloat(d.value.replace(/[^0-9.]/g,"")),0);
  const cur       = industry.currency;
