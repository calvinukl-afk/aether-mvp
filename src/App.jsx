...
Be direct, specific, and commercial. Format with markdown headers and bullets.`;
    ...
    try{
      ...
      if(data.error){
        setMessages(p=>[...p,{role:"assistant",content:"**API Error:** "+data.error.message,files:[]}]);
      }else{
        const text=data.content?.[0]?.text||"Unable to get a response.";
        setMessages(p=>[...p,{role:"assistant",content:text,files:[]}]);
      }
    }catch(e){
      setMessages(p=>[...p,{role:"assistant",content:"**Connection error:** "+e.message,files:[]}]);
    }
    setPending([]);
    setLoading(false);
  };
